...
// Parse scans the given io.Reader and extracts Id, Max and Min into a Rows. Each Row represents a single
...
func Parse(r io.Reader) (Rows, error) {
	...
	for ls.Scan() {
		...
		if _, err := fmt.Fscan(strings.NewReader(l), &row.Id, &row.Max, &row.Min); err != nil {
